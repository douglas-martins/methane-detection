...
_SHARED_SERVING_DIR = str(Path(__file__).resolve().parents[3] / "serving")
if _SHARED_SERVING_DIR not in sys.path:
    sys.path.insert(0, _SHARED_SERVING_DIR)

from band_statistics import BandStats  # noqa: E402

# Matches vendor/starcop/scripts/configs/config.yaml's dataset.input_products
# order for each known registered model (this package's service.py MODEL_NAME).
MODEL_BAND_NAMES = {
