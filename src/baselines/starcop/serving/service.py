...
baseline training's train.py and
src/baselines/starcop/registry/hf_baseline_import.py). The actual
...
_SHARED_SERVING_DIR = Path(__file__).resolve().parents[3] / "serving"
for _module_dir in (_THIS_DIR, _SHARED_SERVING_DIR):
