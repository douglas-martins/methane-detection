...
own pipeline (`src/baselines/starcop/training/train.py`) have a reference point
already in the registry, without reproducing STARCOP's original training run.
...
an actual run instead (the same Test Size: Large boundary as baseline training's
live path; see `internal-docs/setup/environment-notes.md`).
"""
...
for _path in (_SHARED_REGISTRY_DIR, _SHARED_TRAINING_DIR):
...
def import_variant(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with mlflow.start_run(run_name=f"{model_name}-import") as run:
            ...
            # baseline training's train.py, so registered versions are loadable via
