"""Put evaluation, registry, and both training directories on ``sys.path``.

Tests can import modules directly regardless of which order a test file happens
to import them in. Pytest's default import mode
...
_SRC_DIR = _EVALUATION_DIR.parents[2]

for _dir in (
    _EVALUATION_DIR,
    _SRC_DIR / "registry",
    _SRC_DIR / "baselines" / "starcop" / "registry",
    _SRC_DIR / "training",
    _SRC_DIR / "baselines" / "starcop" / "training",
):
    _dir_str = str(_dir)
