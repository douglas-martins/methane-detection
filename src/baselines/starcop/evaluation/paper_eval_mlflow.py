...
_SHARED_REGISTRY_DIR = str(_SRC_DIR / "registry")
_BASELINE_REGISTRY_DIR = str(_SRC_DIR / "baselines" / "starcop" / "registry")
for _registry_dir in (_SHARED_REGISTRY_DIR, _BASELINE_REGISTRY_DIR):
    if _registry_dir not in sys.path:
        sys.path.insert(0, _registry_dir)
_SHARED_TRAINING_DIR = str(_SRC_DIR / "training")
_BASELINE_TRAINING_DIR = str(_SRC_DIR / "baselines" / "starcop" / "training")
for _training_dir in (_SHARED_TRAINING_DIR, _BASELINE_TRAINING_DIR):
...
def build_paper_eval_tags(
    ...
    shared training's mlflow_utils.py::build_run_tags split)."""
    return {
        "variant": variant,
