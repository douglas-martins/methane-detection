...
- **Editing a stage script while its process is running is safe.** Python compiles imported modules into memory at process startup and never re-reads the `.py` file mid-run, so editing `stats.py` on disk while `stats@starcop_raw` was executing had zero effect on that run — the edit (adding a `tqdm` progress bar, `mininterval=5.0` to avoid flooding a redirected log file with updates) only takes effect on the next invocation. One side effect: DVC computes a stage's `dvc.lock` dep hash from the file's state *after* the subprocess exits, not at launch time — so `stats@starcop_raw`'s lock entry ended up recording the *edited* `stats.py` hash even though the code that actually produced its `band_stats.json` was the pre-edit version. Harmless here (the edit was cosmetic, no logic change, tests unaffected), but worth knowing: `dvc status`/`dvc.lock` can't be trusted as proof of exactly which code version produced a given output if you edit a stage's script while it's mid-run. To force a stage to actually pick up an in-flight code edit and re-execute, use `dvc repro --force <stage>`.
