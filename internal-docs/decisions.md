...
The existing 5TB Google plan makes marginal cost $0. The rate-limit risk that made Backblaze B2 attractive is a *shared OAuth quota* problem (the default DVC/pydrive2 app's quota, shared across every DVC user on the internet), not a Drive problem — solved by registering a project-owned OAuth client, historically ~20,000 req/100s dedicated to this project. B2 documented as the fallback, not adopted: full trade-off comparison in `internal-docs/plans/dvc-remote-comparison.md`.

**Fallback triggers** (none hit as of the full 59GB/75,353-file push): sustained 403/429 errors after switching to the dedicated OAuth client; push/pull of the full raw dataset exceeding ~2–3 hours; Drive quota pressure pushing the 5TB plan close to its cap. If triggered, B2 is a drop-in swap (DVC's S3-compatible remote type, one `dvc remote add` + an application key, no pipeline changes).
